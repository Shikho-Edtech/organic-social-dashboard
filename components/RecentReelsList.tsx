        ...
        <>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-ink-100/70 text-[11px] uppercase tracking-wider text-ink-secondary">
                <tr>
              ...
              <tbody>
                {visibleItems.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={row.id + i}
                      className={`border-t border-ink-100 transition-colors hover:bg-indigo-50/30 ${
                        i % 2 === 1 ? "bg-ink-50/40" : ""
                      }`}
                    >
                      <td className="px-4 py-3 text-ink-muted whitespace-nowrap tabular-nums">{row.date}</td>
                      <td className="px-4 py-3 text-ink-primary max-w-[360px]">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums font-semibold text-ink-primary">{row.plays.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right tabular-nums text-ink-300">{row.replays.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right tabular-nums text-ink-secondary">{row.watch}</td>
                      <td className={`px-4 py-3 text-right tabular-nums ${hookColor}`}>{row.hook3}%</td>
                      <td className="px-4 py-3 text-right tabular-nums text-ink-300">{row.replayRate}%</td>
                      <td className={`px-4 py-3 text-right tabular-nums font-semibold ${row.follows > 0 ? "text-brand-green" : "text-ink-300"}`}>
          ...
          <ul className="md:hidden divide-y divide-ink-100">
            {visibleItems.map((row, i) => (
              <li key={row.id + i} className={`px-4 py-3 ${i % 2 === 1 ? "bg-ink-50/40" : ""}`}>
                <div className="flex items-baseline justify-between gap-2 mb-1.5">
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted whitespace-nowrap">
                    {row.date}
                  </div>
                  <span
                ...
                <div className="text-sm text-ink-primary mb-2">
                ...
                <div className="grid grid-cols-3 gap-x-2 gap-y-2 text-xs">
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Plays</div>
                    <div className="text-sm font-semibold text-ink-primary tabular-nums">{row.plays.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Watch</div>
                    <div className="text-sm font-semibold text-ink-primary tabular-nums">{row.watch}s</div>
                  </div>
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Follows</div>
                    <div className="text-sm font-semibold text-brand-green tabular-nums">
                      {row.follows > 0 ? `+${row.follows}` : "0"}
                    </div>
                  </div>
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Hook 3s</div>
                    <div
                  ...
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Replay %</div>
                    <div className="text-sm text-ink-secondary tabular-nums">{row.replayRate}%</div>
                  </div>
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-muted">Replays</div>
                    <div className="text-sm text-ink-secondary tabular-nums">{row.replays.toLocaleString()}</div>
                  </div>
                </div>
              </li>
            ))}
