...
| **T1.12 🆕** | **Join-key vocabulary alignment** — for every join, verify the value vocabularies align across sources, not just column existence. Especially when one side is AI-generated and the other is API-sourced — these are independent vocabularies that drift. Sample 3 actual values from each side; if cardinality / case / spelling diverges, the join silently fails. | ✅ 1 P0 (Plan emitted `Reel/Video/Photo/Carousel` capitalized; FB Graph API emitted `video/photo/carousel` no Reel — every Reel slot was structural no-data even after v4.10) — promoted 2026-05-01 | 🟡 |
| **T1.13 🆕** | **No-fallback for time-windowed reads** — pages taking a time-scope (week, day, run-id) must fail honestly when the requested scope has no data. A fallback that returns `latest` / `any available` / `whichever has data` will silently mislead the user. Empty state with explanation is always better. | ✅ 1 P0 (Plan page silently rendered "the latest week" when Last/Next had no rows — all 3 weekly views looked identical) — promoted 2026-05-01 | 🟡 |
| **T1.14 🆕** | **Cross-half convention audit** — when data flows through two system halves (pipeline write + dashboard read; AI generation + deterministic post-process), every shared key/identifier MUST be defined in exactly one place. If the dashboard re-derives a key the pipeline already stamped, you have a convention bug waiting to fire. Pick any `Week Ending` / `id` / `key` column. Does the dashboard compute its match value from `now()` or read it from the row? If the former, you can produce mismatches. | ✅ 1 P0 (WeekSelector returned closing-Sunday strings while pipeline stored Mon-anchor → empty Diagnosis This-Week, mismatched date pills) — promoted 2026-05-02 | 🟡 |
| **T1.15 🆕** | **Conditional render must have empty-data state** — any UI chip / icon / link gated on optional data (`{maybe && <X/>}`) must have a visible empty-data fallback OR document why disappearance is right. Silent disappearance after a migration looks identical to "feature broken." Empty-state copy ("not yet matched", "data pending", "post not archived") tells the user the feature is alive but the data isn't ready. | ✅ 1 fix (Outcomes PostReference icon disappeared on every Hit row when Matched Post ID was empty in legacy data — looked like the drill-down feature was broken) — promoted 2026-05-02 | 🟡 |
| **T1.16 🆕** | **Schema migration validation step** — after committing a `*_HEADERS` change, before assuming the dashboard will render new columns, run an audit script comparing live sheet header count to code header count. If different, the migration hasn't fired yet. For tabs whose writes are conditional (skipped on AI off, locked on running week, etc.), require an explicit migration step or `force_regenerate` run to push the schema forward. | ✅ 1 fix (Outcome_Log Preliminary/Matched Post ID/Slot Target Metric columns shipped in code but never appeared in sheet because no outcome write fired between commits) — promoted 2026-05-02 | 🟡 |
