...
import { getCalendar, getCalendarByRunId, getCalendarByWeekStarting, getRunStatus, computeStaleness, computeCalendarBannerState, getStageEngine, getPlanNarrative } from "@/lib/sheets";
...
export default async function PlanPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  // aiDisabled=true.
  // 2026-05-05: parallel-render-path consolidation (matches /diagnosis).
  // This page used to have TWO top-level returns (one for the AI-disabled
  // empty state, one for everything else), each duplicating the chrome.
  // Every surgical fix to one branch silently dropped something from the
  // other. Now: ONE return. Chrome props computed up front; only the BODY
  // switches on isEmpty.
  const isEmpty = aiDisabled && !isArchival && calendar.length === 0;

  const headerSubtitle = isArchival
    ? "Archived content calendar"
    : isEmpty
      // Preserve the empty-state copy that was here before consolidation.
      ? "Next week's content calendar — AI generation is off"
      : (isThisWeekView
  ...
  const headerDateLabel = isArchival
    ? "Archived snapshot"
    : isEmpty
      ? "Generated by latest weekly run"
      : `Mon–Sun BDT · ${weekRange(targetWeekStarting)}`;

  // 2026-05-04 incident #5: banner gating via pure function in lib/sheets.
  // In empty-state, hard-code aiDisabled=true to match old behavior.
  const bannerAiDisabled = isEmpty
    ? true
    : computeCalendarBannerState({
        isArchival, isThisWeekView, aiDisabled, calendar,
  ...
  return (
    <div className={isArchival ? "opacity-[0.97] [filter:saturate(0.9)]" : ""}>
      ...
      {/* Sprint P7 v3 (2026-04-29): Plan week selector. Hidden in
          ...
          Renders for empty-state too (parallel-path consolidation 2026-05-05). */}
      {!isArchival && (
      ...
      {/* AI-disabled empty-state body. Chrome above is shared with the
