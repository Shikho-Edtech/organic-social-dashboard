    ...
    <div className={isArchival ? "opacity-[0.97] [filter:saturate(0.9)]" : ""}>
      ...
      {isArchival ? (
        ...
        <StalenessBanner
          ...
          artifact="calendar"
          runStatus={runStatus}
          // 2026-05-04 incident #5: gate via the pure function in lib/sheets
          // so the rule is unit-testable. Banner fires ONLY when AI is off,
          // this-week view, AND no calendar slots exist. When slots exist
          // (typical case — calendar has 30+ slots), the alarming "AI off
          // this run" banner above legit content is noise. Caught by user
          // manual inspection 2026-05-04 (fix shipped on /diagnosis days
          // earlier had not been ported to /plan).
          aiDisabled={computeCalendarBannerState({
